...
class TestClaudeAgentCodeReview:
    ...
    def test_code_review(self, mock_run, agent, temp_file):
        ...
        mock_response = {
            "result": "Security issue found: eval() usage"
        }
        mock_run.return_value = Mock(
            stdout=json.dumps(mock_response),
            returncode=0
...
class TestClaudeAgentFixCode:
    ...
    def test_fix_code(self, mock_run, agent, temp_file):
        """Test code fixing"""
        mock_response = {
            "result": "Fixed: Added try-except block"
        }
        mock_run.return_value = Mock(
            stdout=json.dumps(mock_response),
            returncode=0
...
class TestClaudeAgentIntegration:
    ...
    @pytest.mark.skipif(
        not os.path.exists("/usr/local/bin/claude") and 
        not os.path.exists(os.path.expanduser("~/.local/bin/claude")),
