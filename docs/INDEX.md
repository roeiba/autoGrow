# AutoGrow Documentation Index

> **Central hub for all AutoGrow documentation**

## 📚 Core Documentation

### Getting Started
- **[../README.md](../README.md)** - Quick overview of the .agents folder
- **[SETUP_GUIDE.md](SETUP_GUIDE.md)** - Complete setup instructions
- **[../SECURITY_AUDIT.md](../SECURITY_AUDIT.md)** - Security best practices

### For Users
- **[USER_GUIDE.md](USER_GUIDE.md)** - How to use AutoGrow
- **[TROUBLESHOOTING.md](TROUBLESHOOTING.md)** - Common issues and solutions

### For Contributors
- **[../CONTRIBUTING_GUIDE.md](../CONTRIBUTING_GUIDE.md)** - How to contribute
- **[../MAINTENANCE.md](../MAINTENANCE.md)** - Maintenance procedures

### For AI Agents
- **[../project-rules.md](../project-rules.md)** - Rules for AI code generation

## 🤖 Agent Documentation

### Autonomous Agents
- **[../QA_AGENT.md](../QA_AGENT.md)** - QA Agent documentation
- **[ISSUE_GENERATOR.md](ISSUE_GENERATOR.md)** - Issue Generator docs
- **[ISSUE_RESOLVER.md](ISSUE_RESOLVER.md)** - Issue Resolver docs

## 🔧 Technical Guides

### Configuration
- **[../GITHUB_SECRETS.md](../GITHUB_SECRETS.md)** - GitHub Secrets setup
- **[../github-optimization.md](../github-optimization.md)** - GitHub optimization
- **[../launch-checklist.md](../launch-checklist.md)** - Pre-launch checklist

### Development
- **[ARCHITECTURE.md](ARCHITECTURE.md)** - System architecture
- **[API_REFERENCE.md](API_REFERENCE.md)** - API documentation

## 📜 Historical Documentation

Moved to `../archive/` for reference:
- Migration guides
- Debug summaries
- Fix documentation
- Refactoring notes

## 🛠️ Scripts

Located in `../scripts/`:
- `setup_secrets.sh` - Setup GitHub secrets
- `debug_tools.sh` - Debugging utilities
- `test_agents.sh` - Test agent functionality

## Quick Links

| Topic | Document |
|-------|----------|
| **First Time Setup** | [SETUP_GUIDE.md](SETUP_GUIDE.md) |
| **Security** | [../SECURITY_AUDIT.md](../SECURITY_AUDIT.md) |
| **QA Agent** | [../QA_AGENT.md](../QA_AGENT.md) |
| **Contributing** | [../CONTRIBUTING_GUIDE.md](../CONTRIBUTING_GUIDE.md) |
| **Troubleshooting** | [TROUBLESHOOTING.md](TROUBLESHOOTING.md) |

---

*Last Updated: November 13, 2025*
